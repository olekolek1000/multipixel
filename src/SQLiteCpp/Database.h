 ...
 */
     ...
     */
		Database(const std::string &aFilename,
						 const int aFlags = SQLite::OPEN_READONLY,
						 const int aBusyTimeoutMs = 0,
						 const std::string &aVfs = "") : Database(aFilename.c_str(), aFlags, aBusyTimeoutMs, aVfs.empty() ? nullptr : aVfs.c_str()) {
		}
...
// c++17: Visual Studio 2017 version 15.7
     ...
     */
		Database(const std::filesystem::path &apFilename,
						 const int aFlags = SQLite::OPEN_READONLY,
						 const int aBusyTimeoutMs = 0,
						 const std::string &aVfs = "") : Database(reinterpret_cast<const char *>(apFilename.u8string().c_str()),
