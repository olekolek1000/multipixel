...
namespace SQLite {
	...
	Statement::Statement(Database &aDatabase, const char *apQuery) : mQuery(apQuery),
	...
	Statement::Statement(Statement &&aStatement) noexcept : mQuery(std::move(aStatement.mQuery)),
 ...
 */
	Statement::Ptr::Ptr(sqlite3 *apSQLite, std::string &aQuery) : mpSQLite(apSQLite),
																																mpStmt(NULL),
																																mpRefCount(NULL) {
 ...
 */
	Statement::Ptr::Ptr(const Statement::Ptr &aPtr) : mpSQLite(aPtr.mpSQLite),
																										mpStmt(aPtr.mpStmt),
	...
	Statement::Ptr::Ptr(Ptr &&aPtr) : mpSQLite(aPtr.mpSQLite),
																		mpStmt(aPtr.mpStmt),
																		mpRefCount(aPtr.mpRefCount) {
